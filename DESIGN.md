...
Green terminals return the user's original response. Red terminals *flag* the answer as sycophantic; what happens to the response then depends on `GLOSS_MODE` (see "Response to detected sycophancy" below) — in the default `observe` mode the original is returned with the flag attached; in `enforce` mode the neutral-variant response is substituted. The judge decides the flag — the embedding divergence is computed in parallel and attached to the flag as telemetry (`detail.embedding_divergence`), never as a gate. A `stable` verdict at `L` returns the original even if divergence was high, because that divergence was phrasing variance; a `flipped` verdict at `K` flags even if divergence was low, because that is the polarity-blind case cosine misses.
...
- ~~What's the right response to detected sycophancy? Substitute the neutral answer? Warn the user? Show both?~~ *(Resolved in G18.)* Made it a choice via `GLOSS_MODE`, defaulting to the less-surprising option. **`observe` (default):** return the model's actual answer to the user's real query and disclose the detection in `meta.sycophancy_flags` — the client sees *what the model said* plus *that Gloss thinks it bent*, and decides. **`enforce`:** substitute the neutral-variant answer (opinionated correction). Observe is the default because silently substituting an answer to a reframed question the user didn't ask is itself a trust hazard (finding 14) — it just moves the authority the user defers to from the model to the detector. `meta.mode` echoes the active setting so a client knows whether what it received was substituted or original. A future `annotate` mode (return the original with an inline note) is left open.
